  ...
  try {
    const result = await pool.query(
      `SELECT u.id, u.username, u.display_name, u.is_admin, u.public_key_pem,
              u.is_moderator, u.suspended_at, u.suspended_reason,
              u.silenced_at, u.silenced_reason
       FROM oauth_tokens t
 ...
 *
 * Además corta con 403 si la cuenta está suspendida: un token válido
 * de una cuenta suspendida no debe poder seguir posteando, siguiendo,
 * etc. (Silenciada sí puede seguir logueada — el silencio solo limita
 * su alcance/visibilidad, no le corta el acceso a su propia cuenta.)
 */
async function requireAuth(req, res, next) {
  ...
  if (req.authUser.suspended_at) {
    return res.status(403).json({
      error: 'Esta cuenta está suspendida.',
      reason: req.authUser.suspended_reason || null,
    });
  }
  return next();
}
