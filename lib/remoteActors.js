...
/**
 * Resuelve "usuario@dominio" a la URI real del Actor vía WebFinger
 * (RFC 7033), consultando el servidor de origen directamente. Es el
 * paso que faltaba para poder buscar/seguir a alguien que todavía no
 * tenemos en `remote_actors` (por ejemplo, alguien que Quilltoot nunca
 * vio antes, así que webfinger.js — que solo resuelve NUESTROS propios
 * usuarios — no sirve acá).
 */
async function fetchWebfingerActorUri(username, domain) {
  ...
  if (!response.ok) {
    throw new Error(`WebFinger de ${username}@${domain} respondió HTTP ${response.status}.`);
  }

  const doc = await response.json();
  const selfLink = (doc.links || []).find(
    (l) => l.rel === 'self' && (!l.type || l.type.includes('json'))
  );
  if (!selfLink || !selfLink.href) {
    throw new Error(`WebFinger de ${username}@${domain} no trae un link "self" hacia el Actor.`);
  }
  return selfLink.href;
}

/**
 * Combina WebFinger + resolveRemoteActor: dado "usuario" y "dominio"
 * (sin el "@" inicial, ya separados), devuelve la fila cacheada en
 * `remote_actors`, pidiéndola por primera vez si hace falta.
 */
async function resolveActorByHandle(username, domain) {
  const actorUri = await fetchWebfingerActorUri(username, domain);
  return resolveRemoteActor(actorUri);
}

module.exports = { resolveRemoteActor, fetchActorDocument, upsertRemoteActor, resolveActorByHandle };