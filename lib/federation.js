...
const { isDomainSuspended } = require('./moderation');
...
async function getFollowerInboxUrls(localUserId) {
  // Excluimos followers cuyo dominio esté bloqueado en 'suspend' (no
  // tiene sentido mandarles nada si además los estamos rechazando en
  // el Inbox) y actores puntuales suspendidos individualmente.
  const result = await pool.query(
    `SELECT ra.inbox_url, ra.shared_inbox_url FROM follows f
     ...
     WHERE f.followee_user_id = $1 AND f.status = 'accepted'
       ...
       AND NOT EXISTS (
         SELECT 1 FROM domain_blocks db WHERE db.domain = ra.domain AND db.severity = 'suspend'
       )`,
    [localUserId]
  );
...
async function handleFollow({ follower: followerStub, targetActor }) {
  if (targetActor.suspended_at || (await isDomainSuspended(targetActor.domain))) {
    console.warn(`Follow no enviado: ${targetActor.actor_uri} está suspendido/bloqueado.`);
    return;
  }
  const instanceDomain = getInstanceDomain();
