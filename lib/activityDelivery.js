 ...
 */
// Cuánto esperamos como máximo a que un inbox remoto responda. Sin esto,
// un servidor remoto caído o que simplemente no cierra la conexión deja
// la promesa de fetch() colgada para siempre — y como varias rutas HTTP
// (ej: POST /api/v1/accounts/:id/follow) hacían `await queueFederation(...)`
// antes de responder, el request del cliente (Elk) quedaba "cargando"
// indefinidamente en vez de fallar o completarse.
...
async function deliverActivity({ inboxUrl, activity, keyId, privateKeyPem }) {
  ...
  try {
    ...
    if (err.name === 'AbortError') {
      throw new Error(`Timeout (${DELIVERY_TIMEOUT_MS}ms) entregando ${activity.type} a ${inboxUrl}`);
    }
    throw err;
  } finally {
